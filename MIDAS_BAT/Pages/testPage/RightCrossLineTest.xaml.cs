...
namespace MIDAS_BAT.Pages
{
    ...
    /// </summary>
    public sealed partial class RightCrossLineTestPage : Page
    {
        ...
        public static string TEST_NAME = "rightCrossLineTest";
        public static string TEST_NAME_KR = "사선 따라그리기(오른쪽)";
        public static int TEST_ORDER = 2;

        public RightCrossLineTestPage()
        {
        ...
        private void ResizeCanvas()
        {
            ...
            rightCrossLine.X2 = Math.Cos(Math.PI / 4) * lineWidth;
            rightCrossLine.Y2 = Math.Sin(Math.PI / 4) * lineWidth;
            rightCrossLine.HorizontalAlignment = HorizontalAlignment.Center;
            rightCrossLine.VerticalAlignment = VerticalAlignment.Center;
        ...
        private List<List<DiffData>> calculateDifference()
        {
            ...
            var bounds = ApplicationView.GetForCurrentView().VisibleBounds;
            var ttv = rightCrossLine.TransformToVisual(Window.Current.Content);

            Point l_s = new Point(rightCrossLine.X1 + Math.Cos(-Math.PI / 4) * bounds.Height, rightCrossLine.Y1 + Math.Sin(-Math.PI / 4) * bounds.Height);
            Point l_e = new Point(rightCrossLine.X1 + Math.Cos(Math.PI * 3 / 4) * bounds.Height, rightCrossLine.Y1 + Math.Sin(Math.PI * 3 / 4) * bounds.Height);
            l_s = ttv.TransformPoint(l_s);
            l_e = ttv.TransformPoint(l_e);

            //Point l_s = ttv.TransformPoint(new Point(rightCrossLine.X1, bounds.Height));
            //Point l_e = ttv.TransformPoint(new Point(rightCrossLine.X1, -bounds.Height));
            
            Point orgPoint = ttv.TransformPoint(new Point(rightCrossLine.X1, rightCrossLine.Y1));

            DisplayInformation di = DisplayInformation.GetForCurrentView();
            double unit = Util.mmToPixels(1.0);

            // 121 개의 포인트를 구해서 거리를 본다?
            // 사선을 구해야되기 때문에 
            for (int i = 0; i <= 120; i++)
            {
                Point p1 = l_s;
                Point p2 = l_e;
                p1.X += Math.Cos(Math.PI / 4) * i * unit; 
                p1.Y += Math.Sin(Math.PI / 4) * i * unit;
                p2.X += Math.Cos(Math.PI / 4) * i * unit;
                p2.Y += Math.Sin(Math.PI / 4) * i * unit;
                ...
                if( found )
                    results.Add(new DiffData(String.Format("Num: {0}", i), new Point(orgPoint.X + Math.Cos(Math.PI / 4) * i * unit, orgPoint.Y + Math.Sin(Math.PI / 4) * i * unit), intersectedPoint));
                else
                    results.Add(new DiffData(String.Format("Num: {0}", i), new Point(orgPoint.X + Math.Cos(Math.PI / 4) * i * unit, orgPoint.Y + Math.Sin(Math.PI / 4) * i * unit)));
            }
        ...
        private async Task nextHandling()
        {
            try
            {
                ...
                points.Add(ttv.TransformPoint(new Point(rightCrossLine.X1, rightCrossLine.Y1)));
                points.Add(ttv.TransformPoint(new Point(rightCrossLine.X2, rightCrossLine.Y2)));
