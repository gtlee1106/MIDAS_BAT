...
namespace MIDAS_BAT.Pages
{
    ...
    /// </summary>
    public sealed partial class ClockWiseSpiralTestPage : Page
    {
        ...
        public static string TEST_NAME = "clockWiseSpiralTest";
        public static string TEST_NAME_KR = "나선 따라그리기(시계방향)";
        public static int TEST_ORDER = 3;
        public ClockWiseSpiralTestPage()
        {
            ...
            for (int i = 0; i < m_orgLines.Count; i++)
            {
                ...
                for (int angle = 180; angle < 540; angle += 1)
                {
                    ...
                    if (i < drawSplits.Count)
                    {
                        ...
                        bool found = false;
                        for (int j = drawIdx; j < drawSplits[i].Count - 1; j++) // 가장 마지막점은 빼도록 한다. 그린 선 가장 마지막 점은 보통 다음 바퀴의 시작점임. 
                        {
                            ...
                            if (isIntersected && !(angle == 180 && j == drawSplits[i].Count - 1 - 1)) // 나선 한 바퀴의 가장 마지막에 제일 첫 각도가 걸릴 수 있어서 이를 배제하기 위한 조건
                            {
